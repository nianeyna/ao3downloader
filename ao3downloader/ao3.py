...
from ao3downloader import exceptions, fileio, parse_soup, parse_text, repo, strings
...
def get_work_links_recursive(links_list: list[str], link: str, logfile: str, session: requests.sessions.Session, pages: int, series: bool, visited_series: list[str]) -> None:
    ...
    if parse_text.is_work(link):
        ...
        while True:
            ...
            if pages and pagenum == pages + 1: break
...
def download_series(link: str, filetypes: list[str], folder: str, log: dict, logfile: str, session: requests.sessions.Session, subfolders: bool, images: bool, visited: list[str]=None) -> None:
    ...
    try:
        ...
        if subfolders:
            valid_title = parse_text.get_valid_filename(series_title)
